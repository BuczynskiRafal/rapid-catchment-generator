...
from fuzzy.categories import LandUse, LandForm, Slope, Impervious, Catchments
...
class TestMemberships(unittest.TestCase):
    ...
    def test_membership_has_attributes(self):
        self.assertIsNotNone(self.memberships.land_use_type)
    ...
    def test_membership_antecedent(self):
        self.assertIsInstance(self.memberships.land_use_type, ctrl.Antecedent)
    ...
    def test_memberships_labels(self):
        self.assertEqual(self.memberships.land_use_type.label, "land_use")
    ...
    def test_membership_populate_land_use(self):
        land_use_list = list(vars(LandUse()))
        self.assertIsNotNone(self.memberships.land_use_type)
        self.assertIsNotNone(self.memberships.land_use_type.universe)
        self.assertEqual(list(self.memberships.land_use_type.terms), land_use_list)

    def test_membership_populate_land_form(self):
        land_form_list = list(vars(LandForm()))
        self.assertIsNotNone(self.memberships.land_form_type)
        self.assertIsNotNone(self.memberships.land_form_type.universe)
        self.assertEqual(list(self.memberships.land_form_type.terms), land_form_list)
